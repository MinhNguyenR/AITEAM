...
def test_render_wallet_usage_runs(monkeypatch):
    monkeypatch.setattr("core.dashboard.tui.render.console.print", lambda *args, **kwargs: None)
    render_wallet_usage(DummySummary())


def test_render_session_usage_panel_runs(monkeypatch):
    monkeypatch.setattr("core.dashboard.tui.render.console.print", lambda *args, **kwargs: None)
    monkeypatch.setattr("core.dashboard.tui.render.config.list_workers", lambda: [])
    monkeypatch.setattr(
        "core.dashboard.tui.render.tracker.get_usage_summary",
