...
class TestGetWorker:
    ...
    def test_returns_dict_for_valid_worker(self, cfg):
        with patch("core.cli.state.get_model_overrides", return_value={}), \
             patch("core.cli.state.get_prompt_overrides", return_value={}):
            result = cfg.get_worker("LEADER_MEDIUM")
        assert result is not None
        assert "model" in result

    def test_applies_model_override(self, cfg):
        with patch("core.cli.state.get_model_overrides", return_value={"LEADER_MEDIUM": "custom-model"}), \
             patch("core.cli.state.get_prompt_overrides", return_value={}):
            result = cfg.get_worker("LEADER_MEDIUM")
    ...
    def test_import_error_sets_not_overridden(self, cfg):
        with patch.dict("sys.modules", {"core.cli.state": None}):
            result = cfg.get_worker("LEADER_MEDIUM")
        assert result is not None
