...
from core.cli.python_cli.features.ask.flow import (
    _pick_chat_on_ask_entry,
    looks_like_code_intent,
)
...
from core.cli.python_cli.workflow.runtime.persist.activity_log import (
...
from core.cli.python_cli.features.start.clarification_helpers import (
    is_ambiguous_task,
    generate_clarification_qa,
)
    ...
    *,
    ...
    if not p:
        try:
            ...
            p = ask_with_palette("> ", context="main", header_ansi=ansi).strip()
...
def run_start(
    ...
    *,
    ...
    result = _classify_task(
        prompt, run_ask_mode_cb, project_root, force_mode=force_mode
    )
    if result is None:
        return
    _mode, p = result
