...
def append_workflow_activity(
    ...
    *,
    ...
    if isinstance(detail, dict):
        ...
        detail_text = redact_for_display(str(detail or ""))[:4000]
    rec = {
        "ts": time.time(),
        "level": level,
        "node": node,
...
def format_activity_lines(records: list[dict[str, Any]], needle: str = "") -> list[str]:
    ...
    for r in records:
        ...
        try:
            ...
            tsf = 0.0
        sig = (node, act, det)
        if prev_sig == sig and (tsf - prev_ts) <= 2.0:
            continue
        prev_sig = sig
        prev_ts = tsf
        ...
        if n and n not in blob:
            continue
        tss = time.strftime("%H:%M:%S", time.localtime(tsf)) if tsf else "?"
