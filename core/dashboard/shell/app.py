...
from core.cli.python_cli.shell.prompt import (
    GLOBAL_BACK,
    GLOBAL_EXIT,
    ask_choice,
    normalize_global_command,
)
...
def _panel(title: str, body, *, border: str = _BORDER, out=None) -> None:
    ...
    sink.print(
        Panel(body, title=f"[bold]{title}[/bold]", border_style=border, box=ROUNDED)
    )
...
def _input(prompt: str, *, default: str = "") -> str:
    ...
    return normalize_global_command(
        ask_with_palette(prompt, context="dashboard_home", default=default)
    )
...
def _load_rs() -> DashboardRangeState:
    ...
    try:
        ...
        since = (
            tracker.parse_usage_timestamp(
                str(state.get("dashboard_history_since") or "")
            )
            or None
        )
        until = (
            tracker.parse_usage_timestamp(
                str(state.get("dashboard_history_until") or "")
            )
            or None
        )
        label = str(
            state.get("dashboard_history_label")
            or t("dash.history_label_short").format(n=days)
        )
...
def _build_home_on(settings: dict, rs: DashboardRangeState, out: RichConsole) -> None:
    ...
    try:
        ...
        all_batches = tracker.summarize_tokens_by_cli_batches(
    ...
    if preview:
        ...
        tbl.add_column(
            t("dash.turn"), style=Style(color=PASTEL_CYAN), justify="right", width=6
        )
        ...
        for b in preview:
            reqs = sum(
                int(v.get("requests", 0)) for v in (b.get("by_role") or {}).values()
            )
            tbl.add_row(
                str(b.get("batch_idx", "?")),
        ...
        _panel(
            t("dash.recent_title").format(n=len(batches), label=rs.label),
            tbl,
            border=PASTEL_LAVENDER,
            out=out,
        )
