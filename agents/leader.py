...
Classes:
  - BaseLeader: Abstract base with shared logic
  - LeaderLow:  DeepSeek-V3.2-Speciale  (LOW  - Q&A, skeleton, small fix)
  - LeaderMed:  Kimi-K2.5               (MEDIUM - medium code, train AI, CRUD)
  - LeaderHigh: Gemini-3.1-Pro          (HARD  - architecture, CUDA, systems)


context.md format (required):
  ## 1. DIRECTORY STRUCTURE
  ## 2. FILE MAP          <- each file + function in standard format
  ## 3. DATA FLOW
  ## 4. ATOMIC TASKS
...
from agents.support._leader_format import (
    strip_clarification_blocks,
    trim_to_context_start,
)
...
def _truncate_state(
    state_data: dict, char_limit: int = STATE_CHAR_LIMIT_DEFAULT
) -> str:
    """
...
class BaseLeader(BaseAgent):
    ...
    def generate_context(
        self, state_path: str | Path, *, stream_to_monitor: bool = False
    ) -> str:
        """
        ...
        if not context_content.strip():
            try:
                ...
                pass
            return self._write_no_context(
                state_path, reason="format_output returned empty"
            )
